...
# Install dependencies first (pyproject.toml + the single module it packages)
# so the pip layer is cached independently of test_suite.py edits.
...
# separate from the host's. bridge_mcp.py reads/writes ~/.claude/sessions for
